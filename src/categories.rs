...
//! line: cheap substring tables, evaluated in priority order, with explicit
//! per-binary overrides that always win. There is deliberately no regex
//! dependency — the tables are small, legible, and fast enough to run per
//! frame over hundreds of processes.
//!
...
//! preserved because summaries borrow the underlying processes.
...
impl Category {
    ...
    /// Sort rank for grouped views: user-facing apps first, system last.
    pub fn rank(self) -> u8 {
        match self {
            Self::Browser => 0,
            Self::Electron => 1,
...
impl Default for CategoryConfig {
    fn default() -> Self {
        let table: &[(&[&str], Category)] = &[
            ...
            (
                &[
                    "code", "vscode", "idea", "pycharm", "clion", "webstorm", "zed", "nvim", "vim",
                    "emacs", "helix", "kakoune",
                ],
...
pub fn classify_with(process: &ProcessMemory, config: &CategoryConfig) -> Category {
    ...
    // Kernel threads have no user-space footprint to label.
...
pub fn classify(process: &ProcessMemory) -> Category {
    classify_with(process, &CategoryConfig::default())
}
